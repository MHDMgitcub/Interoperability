...
#pure asking question
def AI_query(query):
    set_groq_api_key()  # Ensure this function is defined elsewhere to set the API key
    ...
    try:
        ...
        chat_completion = client.chat.completions.create(
            messages=[
                {
                    "role": "user",  # Specify the role of the message
                    "content": query,  # Include the user prompt as the message content
                }
            ],
            model="llama3-8b-8192",  # Specify the model to use for the chat completion
        )
